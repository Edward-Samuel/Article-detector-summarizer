...
async def process_content(content: WebContent):
    try:
        ...
        is_article, main_content = article_classifier.predict(content.html_content)
        
        if not is_article:
            return {"is_article": False, "content": None, "summary": None}
            
        # Generate summary
        summary = text_summarizer.summarize(main_content)
